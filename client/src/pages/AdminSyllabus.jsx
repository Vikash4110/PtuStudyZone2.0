...
const AdminSyllabus = () => {
  ...
  const getAllSyllabusData = async () => {
    ...
    try {
      const response = await fetch(`${backendUrl}/api/admin/syllabus`, {
        method: "GET",
        headers: {
  ...
  const deleteSyllabus = async (id) => {
    ...
    try {
      const response = await fetch(`${backendUrl}/api/admin/syllabus/delete/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: authorizationToken,
        },
  ...
  return (
    ...
    <section className="bg-white shadow-md rounded my-6">
      <div className="p-6">
        ...
        <div className="mb-4">
          <Link to="/admin/syllabus/add" className="bg-green-500 text-white py-2 px-4 rounded hover:bg-green-700">
            Add New Syllabus
          </Link>
        </div>
              ...
              {syllabus.map((curService) => (
                <tr key={curService._id} className="text-center">
                  ...
                  <td className="py-2 px-4 border">
                    <Link to={`/admin/syllabus/${curService._id}/edit`} className="bg-blue-500 text-white py-1 px-3 rounded hover:bg-blue-700">Edit</Link>
                  </td>
                  <td className="py-2 px-4 border">
